...
std::vector<uintptr_t> closestEntsArr;
std::vector<float> distancesArr;

float pitchAndYaw[2] = { 0, 0 };
...
Aimbot::Aimbot(HANDLE hProcess, std::vector<uintptr_t> EntityList) {
	...
	for (int I = 0; I < EntityList.size(); I++) {
		...
		ReadProcessMemory(hProcess, (BYTE*)mem::FindAddr(hProcess, EntityList[I], { Player::currentX1 }), &entityPosition[0], sizeof(float), 0);
		ReadProcessMemory(hProcess, (BYTE*)mem::FindAddr(hProcess, EntityList[I], { Player::currentY1 }), &entityPosition[1], sizeof(float), 0);
		ReadProcessMemory(hProcess, (BYTE*)mem::FindAddr(hProcess, EntityList[I], { Player::currentZ1 }), &entityPosition[2], sizeof(float), 0);
		...
		if (distance <= 12.f) {
			closestEntsArr.push_back(EntityList[I]);
			distancesArr.push_back(distance);
		}
	}

	std::sort(distancesArr.begin(), distancesArr.end());

	for (int I = 0; I < closestEntsArr.size(); I++) {
		float entityPosition[3];
		ReadProcessMemory(hProcess, (BYTE*)mem::FindAddr(hProcess, EntityList[I], { Player::currentX1 }), &entityPosition[0], sizeof(float), 0);
		ReadProcessMemory(hProcess, (BYTE*)mem::FindAddr(hProcess, EntityList[I], { Player::currentY1 }), &entityPosition[1], sizeof(float), 0);
		ReadProcessMemory(hProcess, (BYTE*)mem::FindAddr(hProcess, EntityList[I], { Player::currentZ1 }), &entityPosition[2], sizeof(float), 0);
